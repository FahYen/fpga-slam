...
class OdomPipelineLogger {
public:
    static OdomPipelineLogger &Instance() {
        static OdomPipelineLogger logger;
        return logger;
    }

    void Record(int frame_idx,
                const std::string &dataset,
                std::size_t raw_points,
                std::size_t frame_points,
                std::size_t keypoints,
                std::size_t graph_nodes,
                std::size_t graph_edges,
                double load_ms,
                double frontend_ms,
                double queue_push_ms,
                double tf_publish_ms,
                double odom_publish_ms,
                double save_pose_ms,
                double viz_publish_ms,
        ...
        const double denom = wall_total_ms > 0.0 ? wall_total_ms : 1.0;
        ...
        file_ << frame_idx << ','
              ...
              << wall_total_ms << ','
              << (100.0 * load_ms / denom) << ','
              << (100.0 * frontend_ms / denom) << ','
              << (100.0 * queue_push_ms / denom) << ','
              << (100.0 * tf_publish_ms / denom) << ','
              << (100.0 * odom_publish_ms / denom) << ','
              << (100.0 * save_pose_ms / denom) << ','
              << (100.0 * viz_publish_ms / denom) << '\n';

        samples_++;
        wall_total_sum_ += wall_total_ms;
        frontend_sum_ += frontend_ms;
        load_sum_ += load_ms;
        viz_sum_ += viz_publish_ms;
    }

    ~OdomPipelineLogger() {
        if (!enabled_ || !file_.is_open() || samples_ == 0) return;

        const double avg_wall = wall_total_sum_ / static_cast<double>(samples_);
        const double fps = avg_wall > 0.0 ? 1000.0 / avg_wall : 0.0;
        const double denom = wall_total_sum_ > 0.0 ? wall_total_sum_ : 1.0;

        file_ << "# summary_samples=" << samples_ << '\n';
        file_ << "# summary_avg_wall_ms=" << avg_wall << '\n';
        file_ << "# summary_approx_fps=" << fps << '\n';
        file_ << "# summary_frontend_pct=" << (100.0 * frontend_sum_ / denom) << '\n';
        file_ << "# summary_load_pct=" << (100.0 * load_sum_ / denom) << '\n';
        file_ << "# summary_viz_pct=" << (100.0 * viz_sum_ / denom) << '\n';
        file_.flush();

        std::cout << "[ OdomProfile ] samples=" << samples_
                  << " avg_wall_ms=" << avg_wall
                  << " approx_fps=" << fps
                  << " frontend_pct=" << (100.0 * frontend_sum_ / denom)
                  << std::endl;
    }
...
private:
    OdomPipelineLogger() {
        ...
        const char *out_env = std::getenv("SGSLAM_ODOM_PROFILE_OUT");
        output_path_ = (out_env != nullptr) ? std::string(out_env) : std::string("slam_odometry_profile.csv");

        file_.open(output_path_, std::ios::out | std::ios::trunc);
        if (!file_.is_open()) {
            enabled_ = false;
            std::cerr << "[ OdomProfile ] failed to open output file: " << output_path_ << std::endl;
            return;
        }

        file_ << "frame_idx,dataset,raw_points,frame_points,keypoints,graph_nodes,graph_edges,"
              << "load_ms,frontend_ms,queue_push_ms,tf_publish_ms,odom_publish_ms,save_pose_ms,viz_publish_ms,"
              << "compute_total_ms,wall_total_ms,load_pct,frontend_pct,queue_push_pct,tf_publish_pct,"
              << "odom_publish_pct,save_pose_pct,viz_publish_pct\n";
        file_.flush();

        std::cout << "[ OdomProfile ] enabled, writing to " << output_path_ << std::endl;
    }
    ...
    double wall_total_sum_ = 0.0;
    double frontend_sum_ = 0.0;
    double load_sum_ = 0.0;
    double viz_sum_ = 0.0;
};

class MappingPipelineLogger {
public:
    static MappingPipelineLogger &Instance() {
        static MappingPipelineLogger logger;
        return logger;
    }
    ...
    void Record(int frame_idx,
                ...
                bool loop_detected,
                double dequeue_ms,
                double mapping_main_ms,
                double pgo_ms,
                double update_map_ms,
                double publish_map_ms,
        ...
        std::lock_guard<std::mutex> lock(mutex_);
        if (!file_.is_open()) return;
        const double denom = loop_total_ms > 0.0 ? loop_total_ms : 1.0;

        file_ << frame_idx << ','
              << static_cast<int>(had_frame) << ','
              << static_cast<int>(run_isam) << ','
              << static_cast<int>(run_map_update) << ','
              << static_cast<int>(loop_detected) << ','
              << dequeue_ms << ','
              << mapping_main_ms << ','
              << pgo_ms << ','
              << update_map_ms << ','
              ...
              << gen_des_ms << ','
              << search_ms << ','
              << (100.0 * dequeue_ms / denom) << ','
              << (100.0 * mapping_main_ms / denom) << ','
              << (100.0 * pgo_ms / denom) << ','
              << (100.0 * update_map_ms / denom) << ','
              << (100.0 * publish_map_ms / denom) << '\n';

        if (had_frame) {
            samples_++;
            total_sum_ += loop_total_ms;
            mapping_main_sum_ += mapping_main_ms;
            pgo_sum_ += pgo_ms;
            update_sum_ += update_map_ms;
            publish_sum_ += publish_map_ms;
        }
    }

    ~MappingPipelineLogger() {
        if (!enabled_ || !file_.is_open() || samples_ == 0) return;
        const double avg_ms = total_sum_ / static_cast<double>(samples_);
        const double denom = total_sum_ > 0.0 ? total_sum_ : 1.0;
        file_ << "# summary_samples=" << samples_ << '\n';
        file_ << "# summary_avg_loop_ms=" << avg_ms << '\n';
        file_ << "# summary_mapping_main_pct=" << (100.0 * mapping_main_sum_ / denom) << '\n';
        file_ << "# summary_pgo_pct=" << (100.0 * pgo_sum_ / denom) << '\n';
        file_ << "# summary_update_map_pct=" << (100.0 * update_sum_ / denom) << '\n';
        file_ << "# summary_publish_map_pct=" << (100.0 * publish_sum_ / denom) << '\n';
        file_.flush();

        std::cout << "[ MappingProfile ] samples=" << samples_
                  << " avg_loop_ms=" << avg_ms
                  << " mapping_main_pct=" << (100.0 * mapping_main_sum_ / denom)
                  << " pgo_pct=" << (100.0 * pgo_sum_ / denom)
                  << std::endl;
    }
...
private:
    MappingPipelineLogger() {
        ...
        const char *out_env = std::getenv("SGSLAM_MAPPING_PROFILE_OUT");
        output_path_ = (out_env != nullptr) ? std::string(out_env) : std::string("slam_mapping_profile.csv");

        file_.open(output_path_, std::ios::out | std::ios::trunc);
        if (!file_.is_open()) {
            enabled_ = false;
            std::cerr << "[ MappingProfile ] failed to open output file: " << output_path_ << std::endl;
            return;
        }

        file_ << "frame_idx,had_frame,run_isam,run_map_update,loop_detected,dequeue_ms,mapping_main_ms,pgo_ms,"
              << "update_map_ms,publish_map_ms,loop_total_ms,gen_des_ms,search_ms,dequeue_pct,mapping_main_pct,"
              << "pgo_pct,update_map_pct,publish_map_pct\n";
        file_.flush();

        std::cout << "[ MappingProfile ] enabled, writing to " << output_path_ << std::endl;
    }
    ...
    double mapping_main_sum_ = 0.0;
    double pgo_sum_ = 0.0;
    double update_sum_ = 0.0;
    double publish_sum_ = 0.0;
};
...
void Odometry::RegisterFrame() {
    ...
    while(ros::ok()){
        ...
        const auto wall_loop_t0 = std::chrono::steady_clock::now();
        ...
        const double frontend_ms = std::chrono::duration<double, std::milli>(frontend_t1 - frontend_t0).count();
        ...
        const double queue_push_ms = std::chrono::duration<double, std::milli>(queue_t1 - queue_t0).count();
        ...
        const auto tf_pub_t0 = std::chrono::steady_clock::now();
        ...
        const auto tf_pub_t1 = std::chrono::steady_clock::now();
        const double tf_publish_ms = std::chrono::duration<double, std::milli>(tf_pub_t1 - tf_pub_t0).count();

        // Publish odometry msg
        const auto odom_pub_t0 = std::chrono::steady_clock::now();
        ...
        const auto odom_pub_t1 = std::chrono::steady_clock::now();
        const double odom_publish_ms = std::chrono::duration<double, std::milli>(odom_pub_t1 - odom_pub_t0).count();

        // Save odometry result, kitti format
        const auto save_pose_t0 = std::chrono::steady_clock::now();
        ...
        fout_odom_.close();
        const auto save_pose_t1 = std::chrono::steady_clock::now();
        const double save_pose_ms = std::chrono::duration<double, std::milli>(save_pose_t1 - save_pose_t0).count();


        // Publish SG-SLAM internal data, just for debugging and viualization
        const auto viz_pub_t0 = std::chrono::steady_clock::now();
        ...
        const auto viz_pub_t1 = std::chrono::steady_clock::now();
        const double viz_publish_ms = std::chrono::duration<double, std::milli>(viz_pub_t1 - viz_pub_t0).count();

        const auto wall_loop_t1 = std::chrono::steady_clock::now();
        const double wall_total_ms = std::chrono::duration<double, std::milli>(wall_loop_t1 - wall_loop_t0).count();
        OdomPipelineLogger::Instance().Record(cloudInd,
                                              dataset_,
                                              frame_raw.first.size(),
                                              frame.first.size(),
                                              keypoints.first.size(),
...
void Mapping::ScanMapping(){
    ...
    while(ros::ok()&&config_.loop_closure_enable){   // loop clousure enable
        ...
        // Timer
        auto start_time = std::chrono::steady_clock::now();
        double dequeue_ms = 0.0;
        double mapping_main_ms = 0.0;
        double pgo_ms = 0.0;
        double update_map_ms = 0.0;
        double publish_map_ms = 0.0;
        bool run_isam_this_loop = false;
        bool run_map_update_this_loop = false;
        bool loop_detected_this_loop = false;
        int profile_frame_idx = -1;
        ...
        if(!scan_queue.empty()){
            ...
            // Get frame data
            const auto dequeue_t0 = std::chrono::steady_clock::now();
            scan_mutex.lock();
            const auto frame_data = scan_queue.front();
            scan_queue.pop();
            scan_mutex.unlock();
            const auto dequeue_t1 = std::chrono::steady_clock::now();
            dequeue_ms = std::chrono::duration<double, std::milli>(dequeue_t1 - dequeue_t0).count();

            profile_frame_idx = frame_data.cloud_id;
            ...
            const auto mapping_main_t0 = std::chrono::steady_clock::now();
            semGraphMappinger.mainProcess(frame_data.cloud_id, frame_data.graph);
            const auto mapping_main_t1 = std::chrono::steady_clock::now();
            mapping_main_ms = std::chrono::duration<double, std::milli>(mapping_main_t1 - mapping_main_t0).count();
            loop_detected_this_loop = semGraphMappinger.loop_flag;
        ...
        if(run_isam_count >=config_.frame_acc_pgo || seq_finsih_mapping){
            const auto pgo_t0 = std::chrono::steady_clock::now();
            run_isam_this_loop = true;
            run_isam_count = 0;
            ...
            const auto pgo_t1 = std::chrono::steady_clock::now();
            pgo_ms = std::chrono::duration<double, std::milli>(pgo_t1 - pgo_t0).count();
        }
        ...
        if(run_map_update_count>=(config_.frame_acc_pgo)||seq_finsih_mapping){
            ...
            update_map_ms = std::chrono::duration<double, std::milli>(update_t1 - update_t0).count();
            ...
            publish_map_ms = std::chrono::duration<double, std::milli>(publish_t1 - publish_t0).count();
        }
        ...
        MappingPipelineLogger::Instance().Record(profile_frame_idx,
                                                 process_flag_debug,
                                                 run_isam_this_loop,
                                                 run_map_update_this_loop,
                                                 loop_detected_this_loop,
                                                 dequeue_ms,
                                                 mapping_main_ms,
                                                 pgo_ms,
                                                 update_map_ms,
                                                 publish_map_ms,
                                                 duration,
                                                 semGraphMappinger.time_gen_des,
                                                 semGraphMappinger.time_search);
