...
std::vector<float> GenGraphDescriptors(const Graph &graph, double edge_dis_th,int subinterval){
    // TODO(M1-FPGA): Profile this descriptor histogram build separately from ICP to
    // determine whether it should be part of the first accelerator iteration.
...
std::tuple<V3d_i,V3d_i> OutlierPruning(const Graph &graph1, const Graph &graph2, V3d_i match_node1, V3d_i match_node2){
    // TODO(M1-FPGA): Track match_node count distribution on real datasets before
    // attempting hardware mapping; small problem sizes may not amortize transfer overhead.
