...
Before hardware offload, we will profile and optimize SG-SLAM on CPU by parallelizing selected kernels and restructuring their dataflow. Our current target order is:

- `BuildGraph()` / `ReBuildGraph()` edge construction and local descriptor accumulation
- loop-closure association cost-matrix generation in `FindCorrespondencesWithIdx()`
- ICP correspondence search as a later, higher-risk optimization path

Pose graph optimization will remain on CPU initially. For the drone-focused MVP, we will treat PGO as a scheduling and tuning problem (optimization cadence, trigger policy, and keyframe density) rather than a first FPGA kernel.
...
The accelerator phase will focus on dense, regular kernels that are more compatible with HLS dataflow than sparse nonlinear optimization:

- first target: semantic graph edge construction in `BuildGraph()` / `ReBuildGraph()`
- second target: loop-closure cost-matrix generation before the Hungarian solve
- later targets: ICP correspondence search if profiling shows enough remaining headroom

For the MVP, the Hungarian assignment, ICP refinement, and GTSAM-based PGO will stay on CPU.

An initial `BuildGraph()` microbenchmark already shows why this order makes sense: on synthetic `512`-node frames, the baseline path measured `147.05 ms`, and the first software optimization pass reduced it to `93.99 ms`. Even after that improvement, the kernel remains too expensive for a drone-class real-time budget, which makes it the strongest first accelerator candidate.
...
- Clear bottleneck characterization on CPU baselines, including `BuildGraph()` latency-versus-node-count curves and dataset-backed timing hooks for KITTI / MulRAN.
- Quantified gains from software-level parallelization, with `BuildGraph()` established as the first accelerator candidate.
- Pivot decision:
...
- Synthesize and deploy the initial `BuildGraph()` accelerator on AWS FPGA.
- Integrate the accelerator with the modified SG-SLAM pipeline and keep loop-closure assignment / PGO on CPU.
